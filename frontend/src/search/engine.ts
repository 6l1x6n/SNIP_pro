...
function normalizeQuery(q: string): string {
  return q
    ...
    // ё→е до чистки (иначе «проём» → «про ем»), казахские буквы не выбрасываем —
    // иначе kz-запрос («алаңы» → «ала ы») разваливается до токенизации
    .replace(/[^a-zа-яёәғқңөұүһі0-9_\s-]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
